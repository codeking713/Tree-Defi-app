...
contract TreeVault is Ownable, Pausable {
     ...
     * @dev Only callable by the contract admin.
     */
    function setSeedPoolPid(uint256 _seedPoolPid) external onlyAdmin {
    ...
    function calculateHarvestTreeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingTree(0, address(this));
