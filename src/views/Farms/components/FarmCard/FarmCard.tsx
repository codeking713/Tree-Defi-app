...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, treePrice, ethereum, account }) => {
  ...
  return (
    <FCard>
      {(farm.tokenSymbol === 'SEED' || farm.tokenSymbol === 'TREE') && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
      ...
      {(farm.pid === 16 || farm.pid === 17 || farm.pid === 18 || farm.pid === 21) && (
        <div className="ribbon-wrapper">
          <div className="ribbon">
            <Text className="ribbontext" style={{ fontWeight: 'bold', color: 'white' }}>
      ...
      <Flex justifyContent="space-between">
        {farm.pid === 3 && (
          <>
            <Text style={{ fontSize: '24px' }}>{TranslateString(10001001, 'Harvest Fee')}:</Text>
